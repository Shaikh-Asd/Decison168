                ...
                <ol class="breadcrumb m-0">
                    ...
                    <form name="all_project_search_task" id="all_project_search_task" method="post">
                            <div class="form-group mb-2">
                                    <input type="hidden" name="page_name" value="tasks_list_page">
                                    ...
                                    <select class="form-select" name="pid_task" id="pid_task" onchange="return all_project_search_task_submit();" style="width: 180px !important;">
                                        ...
                                        if($plist)
                                        {
                                            foreach($plist as $pl)
                                            {
                                                ...
                                                {
                                        ?>
                                        <option value="<?php echo $pl->pid;?>" <?php if(!empty($this->session->userdata('searched_pid_list'))){ if($this->session->userdata('searched_pid_list') == $pl->pid){ echo "selected";}}?>><?php echo $pl->pname;?></option>
                                        <?php
                                                }
                                            }
                                        }
                                        if($accepted_plist)
                                        {
                                            foreach($accepted_plist as $apl)
                                            {
                                                ...
                                                {
                                        ?>
                                        <option value="<?php echo $apl->pid;?>" <?php if(!empty($this->session->userdata('searched_pid_list'))){ if($this->session->userdata('searched_pid_list') == $apl->pid){ echo "selected";}}?>><?php echo $apl->pname;?></option>
                                        <?php
                                                }
                                            }
                                        }
                                    ...
                                    <?php
                                    if(($this->session->userdata('searched_page_name') == "tasks_list_page"))
                                    {
                                        $this->session->unset_userdata('created_tl_pid');
                                    ?>
                                    <a class="float-end nameLink" style="font-weight: 300 !important;font-size: 12px;" href="<?php echo base_url('tasks-list');?>">Clear Filter</a> 
                                    <?php
                                    }
                                    ?>
                            </div>
...
if($CreatedTaskList)
{
    foreach($CreatedTaskList as $atl)
    {
        $check_pro_createdby = "";
...
{
    $check_tid = "";
    foreach($CreatedSubTaskList as $get_atl)
    {
    if($check_tid != $get_atl->s_tid)
    {
        ...
        if($atl)
        {
            if(($atl->tcreated_by != $this->session->userdata('d168_id')))
            {
            $check_pro_createdby = "";
            if(!empty($atl->tproject_assign))
            {
