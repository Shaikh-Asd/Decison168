                ...
                <ol class="breadcrumb m-0">
                    ...
                    <form name="all_project_search_task" id="all_project_search_task" method="post">
                            <div class="form-group mb-2">
                                    <input type="hidden" name="page_name" value="week_tasks_page">
                                    <!-- <label>Search</label> -->
                                    <select class="form-select" name="pid_task" id="pid_task" onchange="return all_project_search_task_submit();" style="width: 180px !important;">
                                        ...
                                        if($plist)
                                        {
                                            foreach($plist as $pl)
                                            {
                                                $t_avail = "";
                                                $st_avail = "";
                                                $check_t_avail = $this->Front_model->check_week_t_avail($pl->pid,$FirstDay,$LastDay);
                                                if($check_t_avail > 0)
                                                {
                                                    $t_avail = "yes";
                                                }
                                                $check_st_avail = $this->Front_model->check_week_st_avail($pl->pid,$FirstDay,$LastDay);
                                                if($check_st_avail > 0)
                                                {
                                                    $st_avail = "yes";
                                                }
                                        ...
                                        if($accepted_plist)
                                        {
                                            foreach($accepted_plist as $apl)
                                            {
                                                $t_avail2 = "";
                                                $st_avail2 = "";
                                                $check_t_avail2 = $this->Front_model->check_week_t_avail($apl->pid,$FirstDay,$LastDay);
                                                if($check_t_avail2 > 0)
                                                {
                                                    $t_avail2 = "yes";
                                                }
                                                $check_st_avail2 = $this->Front_model->check_week_st_avail($apl->pid,$FirstDay,$LastDay);
                                    ...
                                    <?php 
                                    if($this->session->userdata('searched_page_name') == "week_tasks_page")
                                    {
                                    ?>
                                    <a class="float-end nameLink" style="font-weight: 300 !important;font-size: 12px;" href="<?php echo base_url('week-tasks');?>">Clear Filter</a> 
                                    <?php
                                    }
                                    ?>   
                            </div>
...
<?php
    ...
    else
    {
        $FirstDay = date('Y-m-d', strtotime('+1 day'));
        $LastDay = date('Y-m-d', strtotime($FirstDay .' +6 days'));
        $getTaskSubtaskToCheckList = $this->Front_model->getTaskWEEKSubtaskToCheckList($atl->tid,$FirstDay,$LastDay);
...
if($WeekSubtaskslist_Task)
{
    $check_tid = "";
    foreach($WeekSubtaskslist_Task as $get_atl)
    {
