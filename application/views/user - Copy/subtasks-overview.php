...
if($stdetail)
{
    if(isset($_COOKIE["d168_selectedportfolio"]))
    {
        ...
        {
            setcookie("d168_selectedportfolio",$stdetail->portfolio_id,time()+ (10 * 365 * 24 * 60 * 60),'/');
            header("Refresh:0");
        }
    }
                ...
                <div class="col-9">
                    <div>
                        <ul class="nav nav-pills justify-content-end">
                            ...
                            ?>
                            <li class="nav-item">
                                <a class="btn btn-sm bg-d text-white" href="<?php echo base_url('projects-overview-request/'.$stdetail->stproject_assign);?>">
                            ...
                            ?>
                            <li class="nav-item">
                                <a class="btn btn-sm bg-d text-white" href="<?php echo base_url('projects-overview/'.$stdetail->stproject_assign);?>">
                                    <i class="mdi mdi-keyboard-backspace"></i> Go To Project
                                </a>
                            </li>
                            <?php
                            ...
                            ?>
                            <li class="nav-item">
                                <a class="btn btn-sm bg-d text-white" href="<?php echo base_url('projects-overview-accepted/'.$stdetail->stproject_assign);?>">
                                    <i class="mdi mdi-keyboard-backspace"></i> Go To Project
                                </a>
                            </li>
                            <?php
...
if($privilege_only_view == 'no')
{
                                                    ...
                                                    <input type="hidden" id="timer_sflag_new_<?php echo $stdetail->stid;?>" value="<?php echo $stdetail->sflag;?>">                     
                                                    <input type="hidden"  id="stimer_started_<?php echo $stdetail->stid?>" value="<?php echo $stdetail->sflag?>">   
                                                    <span class="timerSBtn_<?php echo $stdetail->stid;?>" style="margin-left: 25px;font-size: 20px; margin-top: 2px;"><i class="bx bx-play-circle timerSBtn_<?php echo $stdetail->stid;?>" onclick="SubtaskTimer('<?php echo $stdetail->stid;?>');"></i></span> 
                                                    <span class="countersubtask_<?php echo $stdetail->stid;?>" data-id="<?php echo $stdetail->stid;?>" style="margin-left: 10px;font-size: 20px;">
                                                        <?php
                                                        if($stdetail->sflag == '1'){
                                                            if($stdetail->start_stimer_new != "" ){
                                                              // Old time
                                                              $old_time = $stdetail->start_stimer_new;
                                                            }
                                                            else{
                                                            $old_time = $stdetail->start_stimer;
                                                            }
                                                          ...
                                                          // Access the time difference components
                                                            ...
                                                            $timer = $time2->format('H:i:s');
                                                              ...
                                                              if($stdetail->tracked_stime != ""){
                                                              $timer = $stdetail->tracked_stime;
                                                              }
                                                              else{
                                                              $timer = '00:00:00';
                                                              }
                                                    ...
                                                    <input type="hidden" value="<?php echo $stdetail->sflag?>" id="timer_sflag_<?php echo $stdetail->stid;?>">
                                                    <?php
}
                                                    $check_pro_createdby = "";
                                                        if(!empty($stdetail->stproject_assign))
...
<?php
                            ...
                            <div class="col-lg-12">
                                <div class="card">
                                    <div class="card-body">
                                        <h4 class="card-title mb-4">Subtask Links and Comments</h4>
...
{
?>                                                                            
                                                                            <a href="javascript: void(0);" onclick="return PreviewSubtaskFile('<?php echo $stfile_name;?>','<?php echo $stdetail->stid;?>')" title="Preview" class="nameLink">
<?php
}
...
else
{
    if($check_permission)
    {
        ...
        {
        ?> 
            <a href="javascript: void(0);" onclick="return file_preview_access_req('<?php echo $stdetail->stproject_assign;?>')" title="Preview" class="nameLink">
        <?php 
        }
    }
    else
...
{
?>                                                                             
                                                                            <a href="javascript: void(0);" onclick="return PreviewSubtaskFile('<?php echo $stfile_name;?>','<?php echo $stdetail->stid;?>')" title="Preview" class="nameLink"><i class="bx bx-search-alt h3 m-1 text-d"></i></a>
<?php
}
elseif(($pcreated_by == $this->session->userdata('d168_id')) || ($pmanager == $this->session->userdata('d168_id')))
{
...
else
{
    if($check_permission)
    {
        ...
        {
        ?> 
            <a href="javascript: void(0);" onclick="return file_preview_access_req('<?php echo $stdetail->stproject_assign;?>')" title="Preview" class="nameLink"><i class="bx bx-search-alt h3 m-1 text-d"></i></a>
        <?php 
        }
    }
    else
...
{
?>
                                                                            <a href="<?php echo base_url().'front/download_subtaskFileAttachment/'.$stfile_name.'/'.$stdetail->stid;?>" class='text-dark'><i class="bx bx-download h3 m-1 text-d"></i></a>

                                                                            <a href="javascript:void(0)" onclick="return delete_subtaskfile('<?php echo $stfile_name;?>','<?php echo $stdetail->stid;?>');" class="text-dark" title="Delete"><i class="bx bxs-x-square h3 m-1"></i></a>
<?php 
}
?>
                                                                        </div>
...
<script>
    var sid =  $('#stid').val();    
var sflag =  $('#timer_sflag_'+sid).val();  
    if(sflag == '1'){   
        SubtaskTimer3(sid); 
    }
