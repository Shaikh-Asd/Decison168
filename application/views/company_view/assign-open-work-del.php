...
<div class="modal-body m-3 mb-2">
    <p class="text-danger"><?php echo ucfirst($mem_detail->first_name).' '.$mem_detail->last_name;?> have some open work. You have to assign to someone before deleting member!</p>
    <p>Open works:</p>
    <?php
    ...
    ?>
    <form name="OpenWorkNewAssigneeDel" id="OpenWorkNewAssigneeDel" method="post">
    	<input type="hidden" name="old_open_work_assignee_del" value="<?php echo $mem_detail->reg_id;?>">
    	<input type="hidden" name="get_cce_id_to_inactive_del" value="<?php echo $cce_id;?>">
    	<select name="new_open_work_assignee_del" id="new_open_work_assignee_del" class=" form-control pro_team_member" data-placeholder="Assign Open Work..." required>
	    ...
	    <span id="new_open_work_assignee_delErr" class="text-danger"></span>
	    <button type="submit" id="OpenWorkNewAssigneeDelButton" class="btn btn-d btn-sm mt-3 float-end">Assign & Delete</button>
        <img id="dcloader2" style="visibility: hidden;" src="<?php echo base_url()?>assets/images/loading.gif">
	</form> 
    <?php
    }
    ?>       
